#+title: chezmoi-mode

This package provides convenience functions for interacting with the [[https://chezmoi.io/][chezmoi]] dotfile management system via Emacs. These are mostly equivalents to functions like ~find-file~ and ~save-buffer~.

[[https://melpa.org/#/chezmoi-mode][file:https://melpa.org/packages/chezmoi-mode-badge.svg]]

[[file:resources/chezmoi-template-recording.gif]]

The recording above demonstrates editing a chezmoi template with source file on left, command log on top right, and associated target file on bottom right.

* Installation
  Install the main package and the separately packaged Transient menu directly
  from the Git repository with ~use-package~ and its ~:vc~ keyword:

  Go-template source files ending in ~.tmpl~, beginning with ~modify_~, or
  located below ~.chezmoitemplates~ can select their major mode from
  ~chezmoi-template-mode-hook~.  When the resulting buffer has a ~gotmpl~
  Tree-sitter parser, Chezmoi completion and template display are enabled.
  The public
  ~chezmoi-template-normalize-host-filename~ function translates Chezmoi
  source attributes for integrations that infer a host mode from a filename.

  #+begin_src emacs-lisp :noweb yes
(use-package chezmoi-mode
  :vc (:url "https://github.com/chuxubank/chezmoi-mode"
       :rev :newest))

(use-package chezmoi-transient
  :vc (:url "https://github.com/chuxubank/chezmoi-mode"
       :lisp-dir "extensions"
       :main-file "chezmoi-transient.el"
       :rev :newest
       :ignored-files ("chezmoi-age.el"
                       "chezmoi-dired.el"
                       "chezmoi-ediff.el"
                       "chezmoi-magit.el"))
  :commands chezmoi-transient)
  #+end_src

  The optional integrations have separately installable package files and
  descriptors in the same repository.  Install the descriptors that you use:

  #+begin_src emacs-lisp :noweb yes
(use-package chezmoi-age
  :vc (:url "https://github.com/chuxubank/chezmoi-mode"
       :lisp-dir "extensions"
       :main-file "chezmoi-age.el"
       :rev :newest
       :ignored-files ("chezmoi-dired.el"
                       "chezmoi-ediff.el"
                       "chezmoi-magit.el"
                       "chezmoi-transient.el"))
  :autoload (chezmoi-age-get-identity chezmoi-age-get-recipients))

(use-package chezmoi-dired
  :vc (:url "https://github.com/chuxubank/chezmoi-mode"
       :lisp-dir "extensions"
       :main-file "chezmoi-dired.el"
       :rev :newest
       :ignored-files ("chezmoi-age.el"
                       "chezmoi-ediff.el"
                       "chezmoi-magit.el"
                       "chezmoi-transient.el"))
  :commands chezmoi-dired-add-marked-files)

(use-package chezmoi-ediff
  :vc (:url "https://github.com/chuxubank/chezmoi-mode"
       :lisp-dir "extensions"
       :main-file "chezmoi-ediff.el"
       :rev :newest
       :ignored-files ("chezmoi-age.el"
                       "chezmoi-dired.el"
                       "chezmoi-magit.el"
                       "chezmoi-transient.el"))
  :commands (chezmoi-ediff chezmoi-ediff-merge))

(use-package chezmoi-magit
  :vc (:url "https://github.com/chuxubank/chezmoi-mode"
       :lisp-dir "extensions"
       :main-file "chezmoi-magit.el"
       :rev :newest
       :ignored-files ("chezmoi-age.el"
                       "chezmoi-dired.el"
                       "chezmoi-ediff.el"
                       "chezmoi-transient.el"))
  :commands chezmoi-magit-status)
  #+end_src

  In these recipes, ~:main-file~ is relative to ~:lisp-dir~.  The flat shared
  ~extensions~ directory means sibling source files and autoloads remain
  visible on the checkout's load path; it does not isolate their code.
  ~:ignored-files~ only keeps sibling package headers from contributing
  optional dependencies to the selected package descriptor.  Because the
  repository-level ~.elpaignore~ keeps optional extensions out of the main
  package build, Emacs 30 loads these flat extension entry files from source.

  Files below ~chezmoi-root~ enable ~chezmoi-mode~ when visited. Set
  ~chezmoi-auto-enable-mode~ to nil to disable this behavior.

  Template values are displayed by default. Customize
  ~chezmoi-template-display-p~ to nil to keep the source expressions visible.
  The display can also be toggled per buffer with
  ~chezmoi-template-buffer-display~.

  For development, install the package dependencies and run the checks with:

  #+begin_src shell
make install-test-deps
make clean all
make compile-all-extensions
  #+end_src

  ~compile-all-extensions~ additionally checks the optional Magit integration
  when Magit is installed.

  Add keybindings to some functions. Or call using ~M-x <cmd>~.

  #+begin_src emacs-lisp :noweb yes
(global-set-key (kbd "C-c C f")  #'chezmoi-find)
(global-set-key (kbd "C-c C s")  #'chezmoi-write)
  #+end_src

  You can manually call ~M-x chezmoi-mode~ in a source file buffer to turn
  synchronization on or off. Template buffers additionally get completion and
  template rendering (see [[Templates]]). When using ~chezmoi-find~ or
  ~chezmoi-ediff~, ~chezmoi-mode~ is turned on for the source file buffer.

* Usage
  One of the main goals of this package is to allow users to access their files managed by chezmoi and sync changes in the source state with the target state files that are used. Opening source state files is done using ~chezmoi-find~. This opens a buffer for the source state and adds a hook to sync it with the corresponding target state file when changes are saved. However, to prevent overwriting changes that were made to the target state file outside of the source state (such as by auto-generated content), this hook is only added when the source and target states are in sync at the time of opening.

  You can overwrite the target state file manually by calling ~chezmoi-write~ from within the source state buffer. Alternatively, you can overwrite the source state file with the content from the current target state file using ~chezmoi-write~. This can be performed for several files in a row using ~chezmoi-sync-files~.

  Quickly jump to the corresponding target file or source file using ~chezmoi-open-other~.
  ~chezmoi-find-scripts~ uses the same source-opening path as ~chezmoi-find~,
  so template mode selection, completion, display, and synchronization are
  initialized for Chezmoi scripts as well.

** Transient

   The separately installable ~chezmoi-transient~ package keeps Transient and
   the optional integration commands out of the main ~chezmoi-mode~ package.
   ~M-x chezmoi-transient~ is its entry point for common workflows.  It
   groups commands for finding managed files and scripts, applying or syncing
   changes, inspecting status/data/configuration, resolving conflicts, and
   controlling the current Chezmoi buffer.  Enable the ~-f~ infix to force an
   apply from a source file or save an updated source buffer while syncing.

   Ediff and Magit entries appear when their Chezmoi extension libraries are
   available.  The Dired add command appears only in Dired buffers.  These
   entries do not make the optional integrations dependencies of the main
   package.

* Functions

  Some functions provided are direct equivalents to common emacs functions.

  | Function     | chezmoi equivalent | chezmoi-mode               |
  |--------------+--------------------+----------------------------|
  | find-file    | chezmoi edit       | ~chezmoi\vertfind~         |
  | save-buffer  | chezmoi apply      | ~chezmoi\vertwrite~        |
  | diff         | chezmoi diff       | ~chezmoi\vertdiff~         |
  | ediff        | chezmoi merge      | ~chezmoi\vertediff~        |
  | magit-status | chezmoi git status | ~chezmoi\vertmagit-status~ |

* Templates

  chezmoi-mode displays the values of [[https://www.chezmoi.io/user-guide/templating/][chezmoi template]] selectors with text
  properties in template source buffers. You can toggle their display with
  ~M-x chezmoi-template-buffer-display~ or with a configuration variable:

  #+begin_src emacs-lisp :noweb yes
(setq-default chezmoi-template-display-p t)   ;; Display template values in all source buffers.
(setq chezmoi-template-display-p t)           ;; Display template values in current buffer.
(setq-default chezmoi-template-display-p nil) ;; Don't display template values by default.
  #+end_src

* Integrations

** Ediff: Comparing source and target states

   Emacs has a lovely package called ~ediff~ for comparing differences between files which is perfect for when the source and target states get out of sync. Quickly resolve inconsistencies between source and target states using ~chezmoi-ediff~. Or view the raw diff using ~chezmoi-diff~ if you prefer. Pipe this into your own diff workflow using ~(chezmoi-diff 1)~ to give raw diff input to some other diff management tool.


** Completion at point
   In template buffers, ~chezmoi-mode~ installs ~chezmoi-capf~ buffer-locally.
   It uses Emacs' standard completion-at-point protocol and works with
   completion UIs such as Corfu and Company through ~company-capf~. Repeated
   requests reuse template data for
   ~chezmoi-template-completion-cache-duration~ seconds (one second by
   default).

** Dired

   In a Dired buffer, add files to chezmoi by either moving to the file or marking some and calling ~chezmoi-dired-add-marked-files~.

** Magit

   Use ~chezmoi-magit-status~ to jump to the ~magit-status~ for your chezmoi.

** Org



* Configuration

** Evil

   Integrate with evil mode by toggling  template display when entering insert mode.

   #+begin_src emacs-lisp :noweb yes
(defun chezmoi--evil-insert-state-enter ()
  "Run after evil-insert-state-entry."
  (chezmoi-template-buffer-display nil (point))
  (remove-hook 'after-change-functions #'chezmoi-template--after-change 1))

(defun chezmoi--evil-insert-state-exit ()
  "Run after evil-insert-state-exit."
  (chezmoi-template-buffer-display nil)
  (chezmoi-template-buffer-display t)
  (add-hook 'after-change-functions #'chezmoi-template--after-change nil 1))

(defun chezmoi-evil ()
  (if chezmoi-mode
      (progn
        (add-hook 'evil-insert-state-entry-hook #'chezmoi--evil-insert-state-enter nil 1)
        (add-hook 'evil-insert-state-exit-hook #'chezmoi--evil-insert-state-exit nil 1))
    (progn
      (remove-hook 'evil-insert-state-entry-hook #'chezmoi--evil-insert-state-enter 1)
      (remove-hook 'evil-insert-state-exit-hook #'chezmoi--evil-insert-state-exit 1))))
(add-hook 'chezmoi-mode-hook #'chezmoi-evil)
   #+end_src

** Ligatures

   Ligatures don't seem to play nice with overlaid text. When using templates, it is recommended to turn off ~ligature-mode~.

   #+begin_src emacs-lisp :noweb yes
;; Turn off ligatures because they show up poorly.
(add-hook 'chezmoi-mode-hook #'(lambda () (when (require 'ligature)
                                            (ligature-mode (if chezmoi-mode 0 1)))))
   #+end_src

** Org babel tangle

   I find this hook useful for my emacs config files generated through org-tangle.

   #+begin_src emacs-lisp :noweb yes
(add-hook 'org-babel-post-tangle-hook #'chezmoi-write))
   #+end_src

** Spacemacs layer

   Provided here is a sample layer for those who use Spacemacs. Add it by creating a Spacemacs layer called "chezmoi" and create a file in it called "packages.el" with the following code:

   #+begin_src emacs-lisp :noweb yes :results silent
(defconst chezmoi-packages
  '(
    chezmoi-mode)
  "The list of Lisp packages required by the chezmoi layer.")


(defun chezmoi/init-chezmoi-mode ()
  (use-package chezmoi-mode
    :vc (:url "https://github.com/chuxubank/chezmoi-mode"
         :rev :newest)
    :init
    (spacemacs/declare-prefix "f d" "chezmoi")

    (spacemacs/set-leader-keys
      "f d s" #'chezmoi-write
      "f d g" #'chezmoi-magit-status
      "f d d" #'chezmoi-diff
      "f d e" #'chezmoi-ediff
      "f d f" #'chezmoi-find
      "f d i" #'chezmoi-sync-files
      "f d o" #'chezmoi-open-other
      "f d t" #'chezmoi-template-buffer-display
      "f d c" #'chezmoi-mode)

    (when (equalp dotspacemacs-editing-style 'vim)
      (defun chezmoi--evil-insert-state-enter ()
        "Run after evil-insert-state-entry."
        (chezmoi-template-buffer-display nil (point))
        (remove-hook 'after-change-functions #'chezmoi-template--after-change 1))

      (defun chezmoi--evil-insert-state-exit ()
        "Run after evil-insert-state-exit."
        (chezmoi-template-buffer-display nil)
        (chezmoi-template-buffer-display t)
        (add-hook 'after-change-functions #'chezmoi-template--after-change nil 1))

      (defun chezmoi-evil ()
        (if chezmoi-mode
            (progn
              (add-hook 'evil-insert-state-entry-hook #'chezmoi--evil-insert-state-enter nil 1)
              (add-hook 'evil-insert-state-exit-hook #'chezmoi--evil-insert-state-exit nil 1))
          (progn
            (remove-hook 'evil-insert-state-entry-hook #'chezmoi--evil-insert-state-enter 1)
            (remove-hook 'evil-insert-state-exit-hook #'chezmoi--evil-insert-state-exit 1))))
      (add-hook 'chezmoi-mode-hook #'chezmoi-evil))


    (setq chezmoi-template-display-p t) ;; Display template values in all source buffers.

    ;; Turn off ligatures cuz they look bad.
    (add-hook 'chezmoi-mode-hook #'(lambda () (ligature-mode (if chezmoi-mode 0 1))))

    ;; I find this hook useful for my emacs config files generated through org-tangle.
    (defun chezmoi-org-babel-tangle ()
      (when-let ((fle (chezmoi-target-file (buffer-file-name))))
        (chezmoi-write file)))
    (add-hook 'org-babel-post-tangle-hook #'chezmoi-org-babel-tangle)))
   #+end_src
