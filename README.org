...
* Installation
  ...
  located below ~.chezmoitemplates~ use ~go-template-ts-mode~ when no host
  mode has already been inferred.  Other template/host compositions can be
  selected from ~chezmoi-template-mode-hook~.  The public
